...
Now, with `msg()`, strings can be declared directly as constants. The only requirement is that you pass them through `t()` (from `useMessages()` or `getMessages()`) when displaying.
