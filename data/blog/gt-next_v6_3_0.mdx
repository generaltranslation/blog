...
To achieve this, we introduce a new `msg()` function for translating strings anywhere in your codebase. Previously, developers had to pass the `t()` function from `useGT()` or `getGT()` up the call stack in order to translate a string. With `msg()`, you simply wrap the string once, and then pass it through `m()` at render time.
