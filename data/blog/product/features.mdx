...
I'm sure you've experienced some frustrations with internationalization (i18n) before.
For me personally, I've experienced these pain points:

- To integrate an i18n library into my project, I had to spend a long time refactoring my code
- Managing translation entry keys became a continuous source of tech debt
- Translations took a **long time** to complete
- The quality of translations was hard to control
- Some translations broke the UI, while others were missing context

These frustrations are just some personal anecdotes.
...
- Not worry about managing dictionary keys
...
They interface with General Translation infrastructure which comprises of:

- A translation service that is powered by the latest state of the art LLMs
- A platform that lets you manage your translations
- A global CDN that is always available

Combining `gt-next` and `gt-react` with General Translation infrastructure, you get a powerful and flexible one stop i18n solution that works for all React and Next.js apps.

Here's what makes `gt-next` and `gt-react` unique:

## No Hassle Setup

`gt-next` and `gt-react` are **extremely easy to set up**.

Unlike traditional i18n libraries, they integrate seamlessly with your existing codebase.
...
With `gt-next` and `gt-react`, you can just code, without having to worry about separate dictionary items:
...
The GT translation service can translate your app **on-demand** and **in real time**.
...
Optionally, you can add context to your translations to provide additional information or instructions to the GT translation service.

For example, if you want to specify that a translation should have a certain tone, you can do the following:
...
General Translation infrastructure also includes a global CDN, allowing translations to be lazy loaded and served from the closest edge location.
